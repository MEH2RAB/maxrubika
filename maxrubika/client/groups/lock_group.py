...
class LockGroup:
    async def lock_group(self: "maxrubika.Client", group: str):
        """
        Lock a group by disabling SendMessage access.
        ...
        if "SendMessages" not in access_list:
            return Data({"status": "OK", "message": "Group is already locked (SendMessage access disabled)."})

        new_access = [access for access in access_list if access != "SendMessages"]
        await self.set_group_default_access(group_guid, new_access)

        return Data({"status": "OK", "message": "Group locked successfully (SendMessage access disabled)."})